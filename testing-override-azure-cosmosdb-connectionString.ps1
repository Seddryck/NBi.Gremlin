...
if ($env:APPVEYOR)
{
    Write-Host "Replacing the content of the *.user.config file by *.azure.config"

    $content = (Get-Content $root\NBi.Testing.Core.Gremlin\ConnectionString.azure.config -Encoding UTF8) 
